        ...
        for (var i = 0; i < data.length; i++) {
			...
			if(data[i].matches != undefined){
						if(data[i].matches.length > 0){
							list_html += '<div class="titbox gomatch" m_id="'+data[i].matches[0].entry_id+'">';
...
//          	  alert(s.start_articleid);
        ...
        $(".gomatch").click(function(){
        	...
        	var m_url = 'http://www.jingcaishuo.com/mandarin_h5_html/matchs_detail_mandarin/?EntryId='+m_id;
...
//      alert(t.start_articleid);
        var url = t.domin + '/article/list/buy?time=' + Math.random();
//      t.loading();
        $.ajax({
            url: url,
            type: "get",
