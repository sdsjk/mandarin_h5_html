...
function setTime() {
	...
	$(li[0]).attr('id', GetDateStr1(-1));
	
	$(li[1]).text(GetDateStr(0));
	$(li[1]).attr('id', GetDateStr1(0));
	$(li[2]).text(GetDateStr(+1));
	$(li[2]).attr('id', GetDateStr1(+1));
	getData($(li[1]).attr('id'));
	startTime = $(li[1]).attr('id');
}

function GetDateStr(AddDayCount) {
	var dd = new Date();
	dd.setDate(getDayMethod() + AddDayCount); //获取AddDayCount天后的日期 
