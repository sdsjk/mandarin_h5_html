...
function action() {
    ...
    this.portal_url = 'http://www.jingcaishuo.com/mandarin_h5_html/portal_mandarin/';
...
//  		alert(banner[0].target_type);
            var a = "'"+banner[0].target_type+"'";
    		banner_html += '<img onclick="bannerclick(' + a + ','+banner[0].target_id+','+banner[0].h5Url+')" src="'+banner[0].img_url+'">';
    	} else {
    		banner_html += '<div class="bt_bico">';
		 	for(var c = 0; c < banner.length;c++){
		 		banner_html += '<a href="#">1</a>';
		 	}
			banner_html += '</div>';
			banner_html += '<div class="bt_sbox">';
				banner_html += '<ul>';
				    for(var v = 0; v < banner.length;v++){
				    	var a = "'"+banner[v].target_type+"'";
		    	 		banner_html += '<li class="bannerstr" onclick="bannerclick(' + a + ','+banner[v].target_id+','+banner[v].h5Url+')" htstr="'+banner[v].h5Url+'" target_id="'+banner[v].target_id+'" target_type="'+banner[v].target_type+'"><img src="'+banner[v].img_url+'"></li>';
		    	    }			
				banner_html += '</ul>';
				banner_html += '<a href="javascript:;" id="btn_prev"></a> <a href="javascript:;"id="btn_next"></a>';
			banner_html += '</div>';
    	}
    	...
    	if(banner.length == 1){
    	       ...
    	       $("#banners").show();                             
    	       $("#one_banner").hide();
    	       this.banner_js(banner.length);
     	}
    	 

        

//       $(".bannerstr").click(function() {
//       	
//          var htstr = $(this).attr("htstr");
//          var i_id = $(this).attr("target_id");
//          var target_type = $(this).attr("target_type");
////          alert(target_type);
////          alert(i_id);
//      	if(target_type == 'article'){
////      		alert(i_id);
//              app.openArticle(i_id);
//      	} else if (target_type == 'analyst'){
//              app.openAuthor(i_id); 
//      	} else if(target_type == 'h5'){
//      		app.pushToWebView(htstr, 1) ;
//      	}
//      })
    ...
    this.portal = function (portals){
        var i = this;
    	if(portals.length == 0){
    		 return false;
    	}
    	
    	var p_html = '<div class="ent_nav">';
    	for(var g = 0; g < portals.length; g++){
    		p_html += '<dl class="por" htstr="'+portals[g].h5Url+'" id="'+portals[g].id+'" name="'+portals[g].name+'">';
			p_html += '<dt><img src="'+portals[g].img_url+'" alt=""></dt>';
			p_html += '<dd>'+portals[g].name+'</dd>';
			p_html += '</dl>';
    	}
		p_html += '</div>';
		...
		var pdom = $('.ent_nav'), dl = pdom.find('dl'), lend = dl.length, winw = $(document).width();
		dl.width(winw/lend);
...
//          alert(htstr);
            if(htstr == 'undefined'){
            	var po_url = i.portal_url + '?portalId='+id + '&name='+escape(name);
    ...
    this.List = function(data) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            list_html += '<dl onclick="goauthor(' + data[i].author_id + ')" author_id="' + data[i].author_id + '">';
            list_html += '<dt><img src="' + data[i].authorPic + '"></dt>';
			list_html += '<dd class="h38">';
			list_html += '<span class="name">' + data[i].authorName + '</span><span class="tm">' + fmtMydate(data[i].last_modified) + '</span>';
			list_html += '</dd>';
			list_html += '</dl>';
			
//			list_html += '<span class="txt goarticle" id="' + data[i].id + '">';

//			list_html += '<label>1场比赛</label><label class="gg">|</label><label>四中玩法</label>';
...
//      })
    ...
    this.init = function() {
        ...
        var t = this;
//      alert(1);
        var url = t.domin + '/top-header?time=' + Math.random();
...
//              layer.closeAll();
     j_loading.closeL();
                ...
                if (data.code == 0000) {
                    if(t.start_articleid == 0){
                    	t.portal(data.result.data.Portal);
                        t.banner(data.result.data.Banner);
                    }
                    t.List(data.result.artileList.Articles);
                } else {
                    t.alertmsg(data.msg);
                }
...
//alert(getDeviceinfo().type);

function bannerclick(target_type,target_id,h5Url){
//  		    var htstr = $(this).attr("htstr");
//	            var i_id = $(this).attr("target_id");
//	            var target_type = $(this).attr("target_type");
    	if(target_type == 'article'){
            app.openArticle(target_id);
    	} else if (target_type == 'analyst'){
            app.openAuthor(target_id); 
    	} else if(target_type == 'h5'){
    		app.pushToWebView(h5Url, 1) ;
    	}
}
...
function gomatch(id){
	var m_url = 'http://www.jingcaishuo.com/h5_html/matchs_detail_cantonese/?EntryId='+id;
    app.pushToWebView(m_url, 0) ;
}
