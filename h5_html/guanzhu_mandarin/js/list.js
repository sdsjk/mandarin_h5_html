...
function action() {
    ...
    this.gologin = function (){
    	var login_html = '';
		login_html += '<div class="container">';
			login_html += '<div class="main">';
				login_html += '<p class="on">您还没有登录</p>';
				login_html += '<p class="bary">登录后才能使用此功能</p>';
				login_html += '<p class="btn go_login">立即登录</p>';
			login_html += '</div>';
		login_html += '</div>';
//		$('#b_content').html(login_html);
...
//      alert(t.start_articleid);
        var url = t.domin + '/Author/GetFollowedAuhorArticles?time=' + Math.random();
//      t.loading();
        $.ajax({
            url: url,
            type: "get",
