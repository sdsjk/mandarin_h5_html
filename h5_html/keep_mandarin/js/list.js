...
j_loading.openL();
    ...
    this.List = function(data) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            list_html += '<dl>';
            list_html += '<dt><img class="goauthor" onclick="goauthor(' + data[i].author_id + ')" src="' + data[i].authorPic + '"></dt>';
			list_html += '<dd class="h38 goarticle" onclick="goarticle(' + data[i].id + ')">';
			list_html += '<span class="name">' + data[i].authorName + '</span><span class="tm">' + fmtMydate(data[i].last_modified) + '</span>';
			list_html += '</dd>';
			list_html += '</dl>';
			
			list_html += '<span class="txt goarticle" onclick="goarticle(' + data[i].id + ')">';
			...
			if(data[i].matches != undefined){
						if(data[i].matches.length > 0){
							list_html += '<div class="titbox gomatch" onclick="gomatch('+data[i].matches[0].entry_id+')">';
...
//      })
    ...
    this.init = function() {
        ...
        var url = t.domin + '/article/list/collect?time=' + Math.random();
//      t.loading();
        $.ajax({
            url: url,
            type: "get",
            ...
            data: {
            	language: 'M',
                userId: t.getUserId(),
                securityCode: t.getSecurityCode(),
